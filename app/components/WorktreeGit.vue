...
const pullReason = computed(() => {
  if (!state.value?.branch || !state.value.head)
    return 'Pull requires a checked-out branch with commits.'
  ...
  if (state.value.ahead === undefined)
    return 'Upstream is unavailable locally. Fetch to update remote references.'
  if (state.value.ahead && state.value.behind)
    return 'Branches have diverged. Resolve this in your Git tool; Darsena never merges or rebases.'
  if (
    props.runs.some(
      (run) =>
        run.worktree === props.worktree.path &&
        ['starting', 'running', 'stopping'].includes(run.status),
    )
  )
    return 'Stop tasks running in this worktree before pulling.'
  return ''
})
...
<template>
  <section class="worktree-git nuxt-ui-scope" aria-label="Git actions">
    <div class="git-toolbar">
      <AppTooltip :text="'Comparison uses locally known remote references. Fetch to check the remote.'"><div class="tracking" >
      ...
      <div class="git-buttons">
        ...
        <AppTooltip :text="'Fetch all remotes for this project. No working files are changed.'"><UButton color="neutral" variant="soft" icon="i-lucide-cloud-download" :loading="busy === 'fetch'" :disabled="loading || !!busy || !state?.remotes.length"  @click="execute('fetch')">Fetch</UButton></AppTooltip>
        <AppTooltip :text="pullReason || 'Fetch the upstream and fast-forward only this worktree. No merge, rebase or automatic stash.'"><span >
        ...
        <AppTooltip :text="'Refresh local Git status without contacting the remote.'"><UButton color="neutral" variant="ghost" icon="i-lucide-refresh-cw" aria-label="Refresh Git status" :loading="loading" :disabled="!!busy"  @click="refresh" /></AppTooltip>
      </div>
    </div>
