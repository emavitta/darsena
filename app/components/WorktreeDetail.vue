...
function androidStarted(id: string) {
  androidFolder.value = undefined
  emit('androidStarted', id)
}
const actions = computed<DropdownMenuItem[][]>(() => [
  [
    {
      label: 'Open in VS Code',
      avatar: { src: '/apps/vscode.png', alt: '' },
      onSelect: () => emit('open', '.', 'vscode'),
    },
    {
      label: 'Open in Terminal',
      avatar: { src: '/apps/terminal.png', alt: '' },
      onSelect: () => emit('open', '.', 'terminal'),
    },
    {
      label: 'Open in Android Studio',
      avatar: { src: '/apps/android-studio.png', alt: '' },
      onSelect: () => emit('open', '.', 'android-studio'),
    },
  ],
  [
    {
      label: 'Show worktree in Finder',
      icon: 'i-lucide-folder-open',
      onSelect: () => emit('open', '.', 'finder'),
    },
    {
      label: 'Copy worktree path',
      icon: 'i-lucide-copy',
      onSelect: () => emit('copy', props.worktree.path),
    },
  ],
])
</script>
  ...
  <main class="worktree-detail">
    <header class="detail-header">
      ...
      <div class="branch-line">
        <AppIcon name="GitBranch" :size="20" /><AppTooltip :text="
            worktree.branch
        ...
        ></AppTooltip><AppTooltip :text="worktreeStatusHint(worktree)"><span
      ...
      <div class="path-line">
        <AppTooltip :text="worktree.path"><span class="mono" >{{ worktree.path }}</span
        ></AppTooltip>
        <UDropdownMenu :items="actions" :content="{ align: 'end' }">
          <AppTooltip :text="'Open the selected worktree in an application, show it in Finder or copy its path.'"><UButton
            class="nuxt-ui-scope"
            color="neutral"
            variant="ghost"
